...
query = '../test/facb.fa'
searched_genomes = []
for genome in glob.iglob('../test/*2.fasta'):
    ...
    call('tblastx -db ../db/' + genome + ' -query ' + query + ' -out output/' + genome.split('/')[-1][0:4] + query.split('/')[-1][:-3] + '.blast' + ' -max_target_seqs 1 -outfmt "7 sseqid evalue"', shell=True)
    #-evalue 0.0001 Shouldn't matter because we're only going to look at the best hit anyway

matches = []

for output in glob.iglob('output/m*.blast'):
